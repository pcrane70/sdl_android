...
public class SdlAudioPassThruDialog {
    ...
    private static final int MAX_DURATION = 1000000;

    private final TTSChunk mInitialPrompt;
    private final String[] mDisplayLines;
    private final int mDuration;
    private final boolean mAudioMuted;
    private final SamplingRate mSamplingRate;
    private final BitsPerSample mBitsPerSample;
    private final AudioType mAudioType;
    private final ReceiveDataListener mDataListener;
    private final SdlInteractionSender mAPTSender =
            new SdlInteractionSender(SdlPermission.PerformAudioPassThru);
    private final SdlEndAPTInteractionSender mEndAPTSender =
            new SdlEndAPTInteractionSender(SdlPermission.EndAudioPassThru);

    private SdlAudioPassThruDialog(Builder builder){
    ...
    public boolean send(SdlContext context, @Nullable SdlInteractionResponseListener listener){
        ...
        boolean result= mAPTSender.sendInteraction(context.getSdlApplicationContext(),
                createAudioPassThru(), new SdlInteractionSender.SdlDataReceiver() {
            @Override
    ...
    public boolean stopAudioPassThru(SdlContext context, @Nullable SdlInteractionResponseListener listener){
        return mEndAPTSender.sendInteraction(context.getSdlApplicationContext() ,
                new EndAudioPassThru(), null, listener);
    }
    ...
    private class SdlEndAPTInteractionSender extends SdlInteractionSender{
        ...
        private SdlEndAPTInteractionSender(SdlPermission permission) {
            super(permission);
        }

        @Override
        protected boolean isAbleToSendInteraction(SdlPermission permission, SdlContext context) {
            return super.isAbleToSendInteraction(permission, context) && mAPTSender.getIsPending();
        }
    }
